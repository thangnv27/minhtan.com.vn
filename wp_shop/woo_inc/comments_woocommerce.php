...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( get_option('woocommerce_enable_review_rating') == 'yes' ) {
		...
		if ( $count > 0 ) {
			...
			echo '<div class="star-rating" title="'.sprintf(__( 'Đánh giá %s trong 5', 'woocommerce' ), $average ).'"><span style="width:'.( ( $average / 5 ) * 100 ) . '%"><strong itemprop="ratingValue" class="rating">'.$average.'</strong> '.__( 'out of 5', 'woocommerce' ).'</span></div>';
	...
	if ( have_comments() ) :
		...
		$title_reply = __( 'Đánh giá sản phẩm', 'woocommerce' ).' &ldquo;'.$post->post_title.'&rdquo;';

		echo '<p class="noreviews">'.__( 'Gửi đánh giá sản phẩm của bạn tại đây <a href="#review_form" class="inline show_review_form">Gửi Đánh Giá</a>', 'woocommerce' ).'</p>';
	...
	$comment_form = array(
		...
		'comment_notes_after' => '',
		'fields' => array(
			'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Tên', 'woocommerce' ) . '</label> ' . '<span class="required">*</span>' .
		...
		'label_submit' => __( 'Gửi đánh giá', 'woocommerce' ),
		'logged_in_as' => '',
		'comment_field' => ''
	);
